...
from models.EEGwavnet import EEGwavenet
...
def train_func(pat_id, rate, device = device, step=2, dim=32,if_data=True, if_artifact=True):
    ...
    # data = data.reshape(-1, 88, 512)  # (454 * 88, 4, 512)
    ...
    model = EEGwavenet(n_chans = 88, n_classes= 1).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=8e-3, betas=(0.9, 0.999))
    criterion =  nn.BCEWithLogitsLoss()
    save_path = 'BrainBERT/checkpoints/save_models_'+str(if_data)+'_data/rate_'+str(rate)+'/EEGNet/'+'T_'+str(T)+'/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(save_path, 'pat_'+str(pat_id)+'_EEGNet_checkpoint_singlechannel.pt'))
    start_epoch = 0
    train_ls, val_ls = [], []
