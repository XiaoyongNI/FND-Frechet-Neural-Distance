...
for ks_tuple in SK1[cond]['mu']:
    SK.append(ks_tuple[0])
    
for ks_tuple in Kur1[cond]['mu']:
...
for ks_tuple in SK2[cond]['mu']:
    SK.append(ks_tuple[0])
    
for ks_tuple in Kur2[cond]['mu']:
    Kur.append(ks_tuple[0])
...
plt.plot(N, SK[0:len(N)], label=r'Skewness$||\mu||$', marker='o', markersize=10, color=colors['individual'])
plt.plot(N, Kur[0:len(N)], label=r'Kurtosis$||\mu||$', marker='v', markersize=10, color=colors['individual'])
plt.plot(N, BP[0:len(N)], label=r'HFO$||\mu||$', marker='d', markersize=10, color=colors['individual'])
...
plt.ylim(top=plt.ylim()[1] * 1000)  # Increase y-axis upper limit 
plt.tick_params(axis='both', which='major', labelsize=21) 
plt.legend(loc='upper center', ncol=3, fontsize=19)
plt.grid()
plt.tight_layout()
plt.savefig('Mu_check_ieeg_results_handfeatures.png')
