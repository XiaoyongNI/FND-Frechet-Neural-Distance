...
cases = ["ASAR-c", "ASAR-a", "interpolation-c", "interpolation-a", "SVD_AcrossChannels-c", "SVD_AcrossChannels-a"]
# cases = ["ASAR-N", "ASAR-S", "interpolation-N", "interpolation-S", "SVD_AcrossChannels-N", "SVD_AcrossChannels-S"]
data_folder = "/net/inltitan1/scratch2/Xiaoyong/Artifact_cancellation/ethz_data/interp/"
path_clean = [["ASAR_seizure_amp73.npy","ASAR_nonseizure_amp73.npy"],["interpolation_seizure_amp73.npy","interpolation_nonseizure_amp73.npy"],["SVD_AcrossChannels_N1_seizure_amp73.npy","SVD_AcrossChannels_N1_nonseizure_amp73.npy"]]
data_folder_artifact = "/net/inltitan1/scratch2/yuhxie/ethz_data/"
path_artifact = [["amp73/clean_nonseizure2.npy","amp73/clean_seizure2.npy"], ["amp73/mixed_nonseizure2.npy","amp73/mixed_seizure2.npy"]]
k = 0
for i in range(len(path_clean)):
    wav_clean1 = np.load(data_folder+ path_clean[i][0])
    wav_clean2 = np.load(data_folder+ path_clean[i][1])
    #concatenate the two arrays
    wav_clean = np.concatenate((wav_clean1, wav_clean2), axis=0)
    ...
    for j in range(len(path_artifact)):
        # check if k = i*len(path_artifact)+j
        if k == i*len(path_artifact)+j:
            print("Testing case: ", cases[k]) #i*len(path_artifact)+j
        else:
            print("error!")
        
          
        wav_artifact1 = np.load(data_folder_artifact+path_artifact[j][0])
        wav_artifact2 = np.load(data_folder_artifact+path_artifact[j][1])
        # concatenate the two arrays
        wav_artifact = np.concatenate((wav_artifact1, wav_artifact2), axis=0)
        # upsample from 512Hz to 2000Hz
