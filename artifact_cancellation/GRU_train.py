...
def train_func(pat_id, rate, device = device, step=2, dim=32,if_data=True, if_artifact=True):
    ...
    early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(save_path, 'pat_'+str(pat_id)+'_GRU_checkpoint.pt'))
    start_epoch = 0
    train_ls, val_ls = [], []
    ...
    plt.ylabel('Loss')
    plt.legend()
    fig_name = save_path+'Data_'+str(if_data)+'_Rate_'+str(rate)+'_pid_'+str(pat_id)+'T_'+str(T)+'_GRU_'+'_Loss_pretraining.jpg'
