...
# model = EEGNetModel(chans=1, classes=1, time_points=512)

# ckpt_path = 'BrainBERT/checkpoints/save_models_True_data/rate_0/EEGNet/T_1/pat_18_EEGNet_checkpoint_multipat.pt'
...
for N in Ns:
    ...
    for r in range(Repeat_time):
        ...
        # F_avg = torch.tensor(F_avg, dtype = torch.float32)

        # F_avg = model(F_avg.unsqueeze(1).unsqueeze(1))
        # F_avg = F.mean(axis=1)
        print(F_avg.shape)
        # F_avg = F_avg.detach().numpy()
    ...
    np.save("Gaussian_check_results_"+cond+"_brainbert1_N10k_100k.npy", results)
