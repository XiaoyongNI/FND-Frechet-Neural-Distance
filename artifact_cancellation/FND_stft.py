...
def plot_stft(wav, name):
    ...
    # f[-1]=200
    g1 = plt.pcolormesh(t,f,np.squeeze(linear[1,:,:]), shading="gouraud")
    ...
    plt.savefig(f"stft_visualization_{name}.png")
...
path_clean = "ethz_data/clean_seizure2_amp57.npy"
...
inputs_clean = torch.FloatTensor(linear_clean).transpose(1,2)
print(inputs_clean.shape)

# plot_stft(wav_clean, "mixed_nonseizure1_amp53")


path_artifact = "ethz_data/mixed_nonseizure1_amp57.npy"
...
inputs_artifact = torch.FloatTensor(linear_artifact).transpose(1,2)

# plot_stft(wav_artifact, "mixed_nonseizure1")
...
out_clean_flattened = inputs_clean.reshape(inputs_clean.shape[0], -1).numpy()
out_artifact_flattened = inputs_artifact.reshape(inputs_artifact.shape[0], -1).numpy()
