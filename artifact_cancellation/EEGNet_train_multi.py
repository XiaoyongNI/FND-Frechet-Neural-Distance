...
def train_func(rate, device = device, step=2, dim=32,if_data=True, if_artifact=True):
    multi_data = []
    multi_label = []
    for pat_id in pids:
        ...
        labels = np.repeat(labels, channels)  # (454 * 88,)

        num_samples = data.shape[0]
        indices = np.arange(num_samples)
        print('num_samples: ', num_samples, data.shape,labels.shape)

        # 抽取 30% 的索引
        sample_size = int(0.3 * num_samples)
        sampled_indices = np.random.choice(indices, size=sample_size, replace=False)


        print('data shape: ', np.shape(data[sampled_indices]))#(454, 88, 4, 512)
        print('labels shape: ', np.shape(labels[sampled_indices]))#(454,)

        multi_data.append(data[sampled_indices])
        multi_label.append(labels[sampled_indices])
    
    data = np.concatenate(multi_data, axis=0)
    labels = np.concatenate(multi_label, axis=0)
    print('multi data shape: ', np.shape(data))#(454, 88, 4, 512)
    print('multi labels shape: ', np.shape(labels))#(454,)
    ...
    early_stopping = EarlyStopping(patience=10, verbose=True, path=os.path.join(save_path, 'pat_'+str(pat_id)+'_EEGNet_checkpoint_multipat.pt'))
    start_epoch = 0
    train_ls, val_ls = [], []
