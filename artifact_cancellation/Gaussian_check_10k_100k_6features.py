...
results_SK = {"seizure": {'N': [], 'ks': [], 'mu': [], 'var': []},
        "non_seizure": {'N': [], 'ks': [], 'mu': [], 'var': []}}

results_Kur = {"seizure": {'N': [], 'ks': [], 'mu': [], 'var': []},
        "non_seizure": {'N': [], 'ks': [], 'mu': [], 'var': []}}
...
for N in Ns:
    ...
    ks_list_SK, mu_list_SK, var_list_SK = [], [], []
    ks_list_Kur, mu_list_Kur, var_list_Kur = [], [], []
    ...
    for r in range(Repeat_time):
        ...
        # 2. Line Length, Skewness, Kurtosis, High Gamma Power
        out_features = compute_features(X_sub, fs=sample_freq, gamma_band=(70, 150))
        line_length  = out_features[:, 0]
        skewness     = out_features[:, 1]
        kurtosis     = out_features[:, 2]
        hfo_power    = out_features[:, 3]
        # 3. STFT
        ...
        mu_SK = skewness.mean(axis=0)
        var_SK = skewness.var(axis=0)
        ks_stats_SK = kstest(skewness, 'norm', args=(mu_SK,var_SK**0.5)).statistic                    
        ks_list_SK.append(ks_stats_SK)
        mu_list_SK.append(np.linalg.norm(mu_SK))
        var_list_SK.append(np.linalg.norm(var_SK))
        
        mu_Kur = kurtosis.mean(axis=0)
        var_Kur = kurtosis.var(axis=0)
        ks_stats_Kur = kstest(kurtosis, 'norm', args=(mu_Kur,var_Kur**0.5)).statistic
        ks_list_Kur.append(ks_stats_Kur)
        mu_list_Kur.append(np.linalg.norm(mu_Kur))
        var_list_Kur.append(np.linalg.norm(var_Kur))
        
        mu_BP = hfo_power.mean(axis=0)
        var_BP = hfo_power.var(axis=0)
        ks_stats_BP = kstest(hfo_power, 'norm', args=(mu_BP,var_BP**0.5)).statistic
        ks_list_BP.append(ks_stats_BP)
    ...
    np.save("FD_ValidationCheck/Gaussian_check_results_Hjorth_"+cond+"_N10k_100k.npy", results_Hjorth)
    ...
    np.save("FD_ValidationCheck/Gaussian_check_results_LL_"+cond+"_N10k_100k.npy", results_LL)
    
    results_SK[cond]['N'].append(N)
    results_SK[cond]['ks'].append((np.mean(ks_list_SK), np.std(ks_list_SK)/np.sqrt(Repeat_time)))
    results_SK[cond]['mu'].append((np.mean(mu_list_SK), np.std(mu_list_SK)/np.sqrt(Repeat_time)))
    results_SK[cond]['var'].append((np.mean(var_list_SK), np.std(var_list_SK)/np.sqrt(Repeat_time)))
    np.save("FD_ValidationCheck/Gaussian_check_results_SK_"+cond+"_N10k_100k.npy", results_SK)
    
    results_Kur[cond]['N'].append(N)
    results_Kur[cond]['ks'].append((np.mean(ks_list_Kur), np.std(ks_list_Kur)/np.sqrt(Repeat_time)))
    results_Kur[cond]['mu'].append((np.mean(mu_list_Kur), np.std(mu_list_Kur)/np.sqrt(Repeat_time)))
    results_Kur[cond]['var'].append((np.mean(var_list_Kur), np.std(var_list_Kur)/np.sqrt(Repeat_time)))
    np.save("FD_ValidationCheck/Gaussian_check_results_Kur_"+cond+"_N10k_100k.npy", results_Kur)
    ...
    np.save("FD_ValidationCheck/Gaussian_check_results_BP_"+cond+"_N10k_100k.npy", results_BP)
    ...
    np.save("FD_ValidationCheck/Gaussian_check_results_STFT_"+cond+"_N10k_100k.npy", results_STFT)
    ...
    np.save("FD_ValidationCheck/Gaussian_check_results_all_"+cond+"_N10k_100k.npy", results_all)
