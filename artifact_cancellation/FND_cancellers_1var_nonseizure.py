...
path_clean = ["ASAR_nonseizure_amp73.npy","interpolation_nonseizure_amp73.npy","SVD_AcrossChannels_N1_nonseizure_amp73.npy"]
...
for i in range(len(path_clean)):
    wav_clean = np.load(data_folder+ path_clean[i])
    ...
    for j in range(len(path_artifact)):
        ...
        if k == i*len(path_artifact)+j:
            print("Nonseizure testing case: ", cases[k]) #i*len(path_artifact)+j
        else:
            print("error!")
