...
class ChronoNet(nn.Module):
    ...
    def forward(self, x):
        x = x.contiguous().view(-1, 88, 512)
        x = self.inception1(x)
        x = self.inception2(x)
        x = self.inception3(x)
        # print(x.shape)
        x = x.contiguous().view(-1, 64, 96)
        x, _ = self.gru1(x)
        x_res = x
        x, _ = self.gru2(x)
        x_res2 = x
        ...
        x = x.contiguous().view(-1, 96, 64)
        x = F.elu(self.affine1(x))
        x = x.contiguous().view(-1, 1, 96)
        x, _ = self.gru4(x)
